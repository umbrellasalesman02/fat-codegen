...
const mapRowToTodo = (row: TodoRow) => decodeTodo({
  id: row.id,
  title: row.title,
...
const listTodos = Effect.fn("listTodos")(function*() {
  const sql = yield* SqliteClient.SqliteClient
  const rows = yield* sql<TodoRow>`SELECT id, title, completed, created_at, updated_at FROM todos ORDER BY created_at ASC`.pipe(
    Effect.orDie
  )
...
const findTodoById = Effect.fn("findTodoById")(function*(id: string) {
  const sql = yield* SqliteClient.SqliteClient
  const rows = yield* sql<TodoRow>`SELECT id, title, completed, created_at, updated_at FROM todos WHERE id = ${id}`.pipe(
    Effect.orDie
  )
  const row = rows[0]
  if (!row) {
...
const TodoRpcHandlers = TodoRpcs.toLayer(Effect.gen(function*() {
  yield* ensureSchema()
  return TodoRpcs.of({
...
const TodoRpcServer = RpcServer.layer(TodoRpcs).pipe(
